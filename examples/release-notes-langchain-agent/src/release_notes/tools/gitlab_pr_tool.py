...
class GitLabPRTool(BaseTool):
    ...
    description: str = "Fetches all merged merge requests from a GitLab repository since a given date. Input should be repository (owner/repo format or full URL) and since_date (YYYY-MM-DD format)."
    ...
    def _run(self, repository: str, since_date: str) -> List[Dict[str, Any]]:
        ...
        if repository.startswith('https://gitlab.com/'):
            repository = repository.replace('https://gitlab.com/', '')
